...
function run(main, options, callback) {
  ...
  var execCallback = function() {
    !done && callback(null, options);
    done = true;
  };
  var delay = function(stop) {
    clearTimeout(timer);
    timer = !stop && setTimeout(execCallback, 1000);
  };
  var err = '';
...
function execCmd(main, data, options) {
  ...
  if (options.inspectBrk) {
    args.unshift('--inspect-brk' + (options.inspectBrk === true ? '' : '=' + options.inspectBrk));
  } else if (options.inspect) {
    args.unshift('--inspect' + (options.inspect === true ? '' : '=' + options.inspect));
  }
