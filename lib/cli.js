...
function restart(running, options, callback) {
	stop(running, function (conf, opts) {
		setTimeout(function () {
...
function stop(running, options, callback) {
	var data, pid, config;
	running = running || '.running';
	try {
		data = fs.readFileSync(running, 'utf-8').split('\n');
		pid = data[0];
		options = util._extend(parseJSON(data[1]), options);
	...
	isRunning(pid, function (isruning) {
		...
		callback && callback(config || {}, options || {}, isruning);
	});
}
